...
> **Статус 16.09.2026: Шаги 1–8 выполнены и задеплоены** (Strapi-схема, данные на локальном и проде,
> слайд «Посадка», хаб-страница, чат-инструмент, логика расчёта). Ниже — история шагов и остаток.
...
**Шаг 2. Данные растений** ✅ (5 растений: ru — локально и на проде; be — в общем этапе локализации)
**Шаг 3. `calculator-page` + компоненты `calc.defaults` / `calc.faq-item`** ✅
**Шаг 4. Расширение `product`:** `purpose`, `crop` (manyToOne), `packaging` (repeatable) ✅
**Шаг 5. Логика расчёта:** `shared/utils/calc.ts` (площадь → растения → граммы → пачки) ✅
**Шаг 6. Слайд «Посадка»:** `HamburgerGarden.vue` + 3-й таб + 3-я точка (≤ `$tablet`) ✅
**Шаг 7. Чат-инструмент `calcPlanting`** ✅ (работает на сервере с токеном)
**Шаг 8. Хаб-страница** `/{locale}/posadka-i-urozhay` (SEO, JSON-LD) ✅
...
1. **`seedling` / `fertilizing`** ✅ (17.09) — компоненты `calc.seedling` / `calc.fertilizing`, поля в `crop`,
   ...
   ✅ На **проде** данные `seedling`/`fertilizing` и товар «Рассада Томат, 6 шт» заполнены 17.09
   (токен прод-Admin API хранится локально в `frontend/.env` как `STRAPI_PROD_TOKEN`).
2. **Корзина** ✅ (17.09) — `useCartStore.addToCart(..., quantity)` (по умолчанию 1), в списке товаров
   растения кнопка «В корзину» добавляет **N пачек из расчёта** (`packCountFor`: семена — по граммам семян,
   удобрение — по граммам удобрения, рассада — по растениям), состояние «в корзине» — через `useIsInCart`.
   Проверено: 6 м² томата → рассада ×4 (2 клика → 8 шт, бейдж 4→8), удобрение ×6, семена ×1.
3. **`blog ↔ crop`** (M2M) + JSON-LD `HowTo` ✅ (17.09) — `crop.blogs` ↔ `blog.crops` (M2M),
   компонент `calc.how-to-step` + поле `calculator-page.howTo`; хаб отдаёт `HowTo` в `@graph`
   (шаги из Strapi, фолбэк — локализованные `howToSteps`); слайд «Посадка» — блок «Статьи по растению»;
   страница статьи — CTA «Рассчитать посадку» + «Растение: …».
   Попутно исправлен роут блога: `pages/[lang]/blog.vue` → `blog/index.vue` (иначе `blog/[slug].vue`
   не отрисовывался — родительский роут без `<NuxtPage/>`).
   Локальные данные: 2 статьи (Томат/Огурец), 4 шага HowTo. ⚠️ Прод: схема не задеплоена,
   данные (HowTo + связи статей) не заполнены.
4. **Sitemap:** проверить, что новые типы и хаб-страница попадают.
5. **Тех.долг:** `useViewport` SSR-safe.
6. **Крупные отдельные этапы** (записаны в `frontend/docs/plan.md`): **типизация** (`vue-tsc`, 57 ошибок)
   и **локализация ru/be по ВСЕМ данным** (не только новым типам).
