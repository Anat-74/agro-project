...
3. **`blog ↔ crop`** (M2M) + JSON-LD `HowTo` ✅ (17.09) — `crop.blogs` ↔ `blog.crops` (M2M),
   ...
   Локальные данные: 2 статьи (Томат/Огурец), 4 шага HowTo. ✅ Прод (17.09): схема задеплоена,
   HowTo (4 шага) и 2 статьи про томаты/огурцы созданы и связаны с растениями, проверено публичным API.
4. **Sitemap** ✅ (17.09) — источник `server/api/__sitemap__/urls.ts` переписан:
   массив собирается на запрос (был module-level → дубли ссылок), populate переведён на объектный
   синтаксис Strapi v5 (строка `populate=category,image` давала 400 → **товары и подкатегории
   вообще не попадали** в карту), добавлены главные локалей (`/ru`, `/be`) и блог (список + статьи),
   `process.env` → `useRuntimeConfig()` (в проде .env не читается автоматически).
   Итог: 45 URL без дублей — 2 главные, 8 статики, 8 категорий, 18 подкатегорий, 4 товара, 5 блога.
   Рекомендация на этап SEO-правок: убрать `/cartshopping` из карты (страница корзины — не для индекса).
