...
## 7. Известные проблемы деплоя

- **SSL-сертификат:** Plesk использует самоподписанный сертификат. Playwright отклонит его и закроет контекст, если не указать `ignoreHTTPSErrors: true`.
- **Порт 8443:** Иногда временно не отвечает. Нужны ретраи с паузой.
- **Домен:** В поиске может всплыть createx.vh324.by3020.ihb.by вместо нужного. Проверять URL на `/id/589/`.
- **Инструменты:** Использовать `node scripts/deploy.mjs`. MCP `plesk-deploy_browser_*` не поддерживает ignoreHTTPSErrors.
