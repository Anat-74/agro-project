...
1. **Рефакторинг `ShowHamburger`**: табы «Категории/Меню» через `USlider` (+свайп), поиск в шапке
   панели на всю ширину, `useVoiceInput` (Web Speech API из чата), контакты/соцсети перед Профилем.
...
7. **Новая структура — для всех экранов** (mobile + desktop), не только для мобильного.
...
- Desktop: панель имеет фикс. ширину `--catalog-width` — табы/поиск в ней умещаются?
