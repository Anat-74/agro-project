...
// Ховер-карточка контактов пункта навигации (шапка, desktop).
// Открытие/закрытие — через глобальное состояние isContacts (provide/inject из app.vue),
// триггер — ссылка «Контакты» в BaseNavigation. Самостоятельного триггера нет.
// Самостоятельный попап контактов (по клику, для панели) — ContactsPopover.vue.
// Состояние глобальное, потому что на isContacts реагируют ещё HeroSection и
// HeroGrids (приглушают hero, пока курсор на «Контактах»)
const { isContacts } = inject<VisibilityState>(VISIBILITY_KEY)!;

const popoverRef = useTemplateRef<HTMLElement>("nav-contacts-popover");
...
<template>
  ...
  <div ref="nav-contacts-popover" popover="manual" class="nav-contacts-popover">
    <div class="nav-contacts-popover__card">
      <div
        v-for="phone in phones"
        :key="phone.documentId || phone.id"
        class="nav-contacts-popover__link"
      >
