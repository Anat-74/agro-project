...
// Модалка товара — по паттерну проекта (как Order/Checkout). Открывается
// НЕмодально (show(), useShowMethod: false), поэтому позиционируется относительно
// ближайшего позиционированного предка, а не вьюпорта: из шапки/корзины это даёт
// правильный вид, а внутри панели «Посадка» (её <dialog> в top-layer) сетка
// пересчитывается от панели — миниатюры галереи наезжают на заголовок.
// Попытка перевести на showModal() ломала открытие (конфликт с атрибутом open
// у смонтированного <dialog>): нужен замер в панели и аккуратное решение.
.product-modal {
